...
1. **CLI flag mapping.** Determine how the assets reach the selected Seedance model through the PixVerse CLI and bind each to a positional label:
   ...
   - **Seedance 2.5 output parameters** → T2V accepts `--aspect-ratio auto`. Reference with video defaults to `--duration auto`, which locks `--aspect-ratio auto`; selecting a fixed `--duration 4`–`30` restores both automatic and fixed aspect-ratio choices. Reference without video rejects `--duration auto`.
